...
class CommonCache {
    ...
    public static function instance($app_id){
        ...
        {
            self::$instances[$app_id] = new self($app_id);
        }
     ...
     */
    function __construct($app_id,$dir = '/data/server/workerman-jsonrpc/cache/', $lifetime = 60) {
        if ($this->dir_isvalid ( $dir )) {
            $this->dir = $dir;
            $this->lifetime = $lifetime;
            $this->ext = '.json';
            $this->cacheid = $this->getcacheid ($app_id);
        }
    }
    /**
     * 检查缓存是否有效
    ...
    public function load() {
        ...
        if ($this->isvalid ()) {
            // 以下两种方式,哪种方式好?????
            $data = file_get_contents($this->cacheid);
        }
        return $data;
    }
    /**
     ...
     * 取得缓存文件路径
     */
    private function getcacheid($key='wx_access_token') {
